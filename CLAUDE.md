...
- **Main window toolbar:** SMB connect/disconnect control (connect → spinner/abort → disconnect states) and an About action.
- **Exactly two filesystem views** (fixed — see ADR 0003), each with its own path box, case-insensitive plain-text search filter, and a status bar showing Selected/Visible/Total counts. Each view keeps the full file list in memory but only displays entries matching the filter. Folders sort to the top. Columns: icon, name, size, date modified, hard-link count, inode number.
- **Match Finder panel** (right side of the main horizontal splitter): searches the two views' paths for candidate matches and links the checked results via "Link Selected Matches" — the only way to create hard links in the app (see `src/ui/MatchFinderPanel`, `src/core/LinkRunner`).
